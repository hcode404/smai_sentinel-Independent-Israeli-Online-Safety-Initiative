  ...
  <div class="m-h"><span>${avatar({ ...u, id:userId },'l')}</span>
    <div style="flex:1"><h3 style="margin:0">${esc(u.name||'משתמש')}
      ...
      <div class="row" style="gap:6px;margin-top:5px">${rankBadge(u.rank)}
        ...
        <span class="b b-gray">${ic('clock',11)} מאז ${fmtDate(u.createdAt)}</span></div><div class="tiny mute" style="margin-top:6px">${u.lastSeenAt?'נראה לאחרונה '+fmtDate(u.lastSeenAt)+' בשעה '+fmtTime(u.lastSeenAt):'מצב פעילות מוסתר'}</div></div></div>
  <div class="m-b">
