...
function userMenu(){
  ...
  $$('[data-presence]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await Store.update('users',u.id,{presenceMode:button.dataset.presence});await Auth.refresh();closeModal();renderNav();toast(`הסטטוס שונה ל${PRESENCE[button.dataset.presence].label}`,'ok');}catch(error){button.disabled=false;toast(error.message||'לא ניתן לשנות סטטוס','err');}});
}
